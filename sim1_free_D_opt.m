...
% These are the values used by the three solvers in this comparison.
...
% Fmincon trust-region/SQP baseline.
tr_sqp = struct();
tr_sqp.maxIter = 80;
...
fprintf('Running free-flight D-opt with %d Monte Carlo trials.\n', mc_trials);
...
parfor trial_idx = 1:mc_trials
    ...
    scene = func_init_free(trial_seed, M, sigma2, eps_s, reg_eps, initMaxTry, big_center, big_size, target_offset_z, target_size, beta_max_deg);
    ...
    J0 = (J0 + J0.') / 2;
    f0 = -log(det(J0 + reg_eps * eye(size(J0))));

    % Run the three algorithms from exactly the same initial deployment.
    out_gs_sqp = func_gs_sqp(scene, 'dopt', sigma2, eps_s, reg_eps, maxIter, stop_rel, beta_max_deg, gs_sqp);
    out_iso_sqp = func_iso_sqp(scene, 'dopt', sigma2, eps_s, reg_eps, maxIter, stop_rel, beta_max_deg, generic_mm);
    out_manopt = func_manopt(scene, trial_seed, 'dopt', sigma2, eps_s, reg_eps, manopt);
    out_tr_sqp = func_tr_sqp(scene, trial_seed, 'dopt', sigma2, eps_s, reg_eps, tr_sqp);
    out_pso = func_pso(scene, trial_seed, 'dopt', sigma2, eps_s, reg_eps, pso);
    out_de = func_de(scene, trial_seed, 'dopt', sigma2, eps_s, reg_eps, de);
...
scene_col = repmat("free_space", n_rows, 1);
criterion_col = repmat("dopt", n_rows, 1);
...
summary_scene = repmat("free_space", n_methods, 1);
summary_criterion = repmat("dopt", n_methods, 1);
...
ylabel('-log(det(J))', 'FontSize', 8);
title('free-flight D-opt', 'FontSize', 8, 'FontWeight', 'normal', 'Interpreter', 'none');
...
title('free-flight D-opt runtime', 'FontSize', 8, 'FontWeight', 'normal', 'Interpreter', 'none');
...
results = struct();
results.case_name = 'free-flight D-opt';
results.scene_name = 'free_space';
results.criterion = 'dopt';
...
writetable(trial_table, fullfile(output_dir, 'sim1_free_D_opt_trials.csv'));
writetable(summary_table, fullfile(output_dir, 'sim1_free_D_opt_summary.csv'));
savefig(fig_history, fullfile(output_dir, 'sim1_free_D_opt_history.fig'));
savefig(fig_runtime, fullfile(output_dir, 'sim1_free_D_opt_runtime.fig'));
func_save_figure(fig_history, fullfile(output_dir, 'sim1_free_D_opt_history.png'), 600);
func_save_figure(fig_runtime, fullfile(output_dir, 'sim1_free_D_opt_runtime.png'), 600);
results.history_fig = fullfile(output_dir, 'sim1_free_D_opt_history.fig');
results.runtime_fig = fullfile(output_dir, 'sim1_free_D_opt_runtime.fig');
results.history_png = fullfile(output_dir, 'sim1_free_D_opt_history.png');
results.runtime_png = fullfile(output_dir, 'sim1_free_D_opt_runtime.png');
save(fullfile(output_dir, 'sim1_free_D_opt_results.mat'), 'results', 'trial_table', 'summary_table', 'trial_results');
