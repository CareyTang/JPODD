...
fprintf('Running free-flight A-opt with %d Monte Carlo trials.\n', mc_trials);
...
parfor trial_idx = 1:mc_trials
    ...
    scene = func_init_free(trial_seed, M, sigma2, eps_s, reg_eps, initMaxTry, ...
        big_center, big_size, target_offset_z, target_size, ...
        beta_max_deg);
    ...
    out_gs_sqp = func_gs_sqp(scene, 'aopt', sigma2, eps_s, reg_eps, ...
        maxIter, stop_rel, beta_max_deg, gs_sqp);
    out_iso_sqp = func_iso_sqp(scene, 'aopt', sigma2, eps_s, reg_eps, ...
        maxIter, stop_rel, beta_max_deg, generic_mm);
    out_manopt = func_manopt(scene, trial_seed, 'aopt', sigma2, eps_s, reg_eps, ...
        manopt);
    out_tr_sqp = func_tr_sqp(scene, trial_seed, 'aopt', sigma2, eps_s, reg_eps, ...
        tr_sqp);
    out_pso = func_pso(scene, trial_seed, 'aopt', sigma2, eps_s, reg_eps, ...
        pso);
    out_de = func_de(scene, trial_seed, 'aopt', sigma2, eps_s, reg_eps, ...
        de);
...
title('free-flight A-opt', 'FontSize', 8, 'FontWeight', 'normal', 'Interpreter', 'none');
...
title('free-flight A-opt runtime', 'FontSize', 8, 'FontWeight', 'normal', 'Interpreter', 'none');
...
results = struct();
results.case_name = 'free-flight A-opt';
...
writetable(trial_table, fullfile(output_dir, 'sim1_free_A_opt_trials.csv'));
writetable(summary_table, fullfile(output_dir, 'sim1_free_A_opt_summary.csv'));
savefig(fig_history, fullfile(output_dir, 'sim1_free_A_opt_history.fig'));
savefig(fig_runtime, fullfile(output_dir, 'sim1_free_A_opt_runtime.fig'));
func_save_figure(fig_history, fullfile(output_dir, 'sim1_free_A_opt_history.png'), 600);
func_save_figure(fig_runtime, fullfile(output_dir, 'sim1_free_A_opt_runtime.png'), 600);
results.history_fig = fullfile(output_dir, 'sim1_free_A_opt_history.fig');
results.runtime_fig = fullfile(output_dir, 'sim1_free_A_opt_runtime.fig');
results.history_png = fullfile(output_dir, 'sim1_free_A_opt_history.png');
results.runtime_png = fullfile(output_dir, 'sim1_free_A_opt_runtime.png');
save(fullfile(output_dir, 'sim1_free_A_opt_results.mat'), 'results', 'trial_table', 'summary_table', 'trial_results');
