function scene = func_init_constrained(seed, M, sigma2, eps_s, reg_eps, initMaxTry, ...
    big_center, big_size, target_offset_z, target_size, vertex_box_center, ...
    vertex_cube_size, vertex_box_size, beta_max_deg)
	if M ~= 8
	    error('The constrained cube scenario currently assumes M = 8 sensors.');
	end
	...
	vertex_signs = [ ...
	    ...
	    -1,  1,  1;
	     ...
	     1,  1, -1;
	     1,  1,  1].';
	vertex_centers = vertex_box_center(:) + 0.5 * vertex_cube_size(:) .* vertex_signs;
	P_lb_each = vertex_centers - 0.5 * vertex_box_size(:);
	P_ub_each = vertex_centers + 0.5 * vertex_box_size(:);
	...
	scene = struct();
	scene.scene_name = 'cube';
	scene.vertex_centers = vertex_centers;
	scene.x0 = x0;
	scene.P0 = P0;
