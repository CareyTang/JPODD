function out = func_gs_sqp(scene, criterion, sigma2, eps_s, reg_eps, maxIter, ...
    stop_rel, beta_max_deg, gs_sqp)
	...
	update_p = true;
	update_u = true;
	if isfield(gs_sqp, 'update_p')
	    update_p = gs_sqp.update_p;
	end
	if isfield(gs_sqp, 'update_u')
	    update_u = gs_sqp.update_u;
	end
	...
	hist_eta_r = nan(maxIter, M);
	hist_eta_t = nan(maxIter, M);
	...
	for t = 1:maxIter
	    ...
	    for j = 1:M
	        ...
	        J_minus_j = J_total - sensorData(j).Ji;
	        S_true = (J_minus_j + J_minus_j.') / 2;
	        p_old = P(:, j);
	        u_old = U(:, j);
	        lb = scene.P_lb_each(:, j);
	        ub = scene.P_ub_each(:, j);

	        [p_new, u_new, info, state(j)] = func_update_sensor(p_old, u_old, S_true, lb, ub, x0, ...
	            sigma2_j, criterion, eps_s, reg_eps, beta_max_deg, gs_sqp, state(j), update_p, update_u);
	        P(:, j) = p_new;
	        U(:, j) = u_new;
	        ...
	        J_total = S_true + Ji_new;
	        J_total = (J_total + J_total.') / 2;
	        ...
	        hist_eta_r(t, j) = info.eta_r;
	        hist_eta_t(t, j) = info.eta_t;
	...
	out = func_output('gs_sqp', 'GS-SQP', scene, P, U, P_init, U_init, P_hist, U_hist, ...
	    hist_f, hist_min_eigJ, hist_min_sin, hist_block_iter, hist_exitflag, hist_model_attempt, hist_step_norm, n_sweeps, toc(t0));
	out.hist_eta_r = hist_eta_r(1:n_sweeps, :);
	out.hist_eta_t = hist_eta_t(1:n_sweeps, :);
	out.hist_tau = hist_tau(1:n_sweeps, :);
end

function [p_new, u_new, info, state] = func_update_sensor(p, u, S_true, lb, ub, x0, ...
    sigma2_i, criterion, eps_s, reg_eps, beta_max_deg, gs_sqp, state, update_p, update_u)
	info = func_info();
	info.eta_r = state.eta_r;
	info.eta_t = state.eta_t;
	info.tau = state.tau;
	p_new = p;
	u_new = u;
	if ~update_p && ~update_u
	    return;
	end

	[phi, grad, model, u_base, ok] = func_reduced_eval(p, u, S_true, x0, sigma2_i, ...
	    criterion, eps_s, reg_eps, beta_max_deg, update_u);
	u_new = u_base;
	if ~ok || ~update_p
	    return;
	end

	[eta_r, eta_t] = func_secant_eta(state, p, grad, model.Pr, model.Pt, gs_sqp);
	info.eta_r = eta_r;
	info.eta_t = eta_t;
	state.has_prev = true;
	state.p_prev = p;
	...
	if norm(grad) <= gs_sqp.gradTol
	    return;
	end

	tau = max(state.tau, gs_sqp.tau0);
	step0 = func_project_step(state.qp_step, lb - p, ub - p);
	for attempt = 1:gs_sqp.qpMaxAttempts
	    H0 = eta_r * model.Pr + eta_t * model.Pt + ...
	        2 * model.lambda * (model.H.' * model.W * model.H);
	    [H, tau_used] = func_regularize(H0, tau, gs_sqp);
	    [step, exitflag, iter_used] = func_quadprog(H, grad, lb - p, ub - p, step0, gs_sqp);
	    info.tau = tau_used;
	    info.exitflag = exitflag;
	    info.iter_used = iter_used;
	    info.model_attempt = attempt;
	    if ~all(isfinite(step))
	        tau = func_next_tau(tau_used, gs_sqp);
	        step0 = zeros(3, 1);
	        continue;
	    end
	    ...
	    p_try = p + step;
	    [phi_try, ~, ~, u_try, ok_try] = func_reduced_eval(p_try, u_base, S_true, x0, ...
	        sigma2_i, criterion, eps_s, reg_eps, beta_max_deg, update_u);
	    if ok_try && phi_try <= phi + gs_sqp.acceptTol * max(1, abs(phi))
	        p_new = p_try;
	        u_new = u_try;
	        state.tau = tau_used;
	        state.qp_step = step;
	...
	state.tau = min(gs_sqp.tauMax, tau);
	state.qp_step = zeros(3, 1);
end

function [eta_r, eta_t] = func_secant_eta(state, p, grad, Pr, Pt, gs_sqp)
	eta_r = state.eta_r;
	eta_t = state.eta_t;
	if ~state.has_prev
	    return;
	...
	ds = p - state.p_prev;
	dy = grad - state.g_prev;
	eta_r = func_one_eta(ds, dy, Pr, eta_r, gs_sqp);
	eta_t = func_one_eta(ds, dy, Pt, eta_t, gs_sqp);
end

function eta = func_one_eta(ds, dy, Pdir, eta0, prm)
	Pds = Pdir * ds;
	den = real(Pds.' * Pds) + prm.secantEps;
	eta = eta0;
	if den > prm.secantEps
	    eta_try = real((dy.' * Pds) / den);
	    if isfinite(eta_try) && eta_try > 0
	        eta = max(eta_try, prm.minDirectionalCurvature);
	    end
	end
end
...
function [phi, grad, model, u_star, ok] = func_reduced_eval(p, u, S_true, x0, sigma2_i, ...
    criterion, eps_s, reg_eps, beta_max_deg, update_u)
	if update_u
	    u_star = func_update_orientation(p, u, S_true + reg_eps * eye(3), x0, sigma2_i, ...
	        criterion, beta_max_deg);
	else
	    u_star = u;
	end
	...
	[grad, model] = func_gradient_model(p, u_star, S_true, x0, sigma2_i, ...
	    criterion, eps_s, reg_eps);
	ok = isfinite(phi) && aux.sin_theta >= eps_s && all(isfinite(grad));
end

function [grad, model] = func_gradient_model(p, u, S_true, x0, sigma2_i, ...
    criterion, eps_s, reg_eps)
...
function out = func_output(method_id, method_name, scene, P, U, P_init, U_init, P_hist, U_hist, ...
    hist_f, hist_min_eigJ, hist_min_sin, hist_block_iter, hist_exitflag, hist_model_attempt, ...
    hist_step_norm, n_sweeps, time_sec)
	out = struct();
